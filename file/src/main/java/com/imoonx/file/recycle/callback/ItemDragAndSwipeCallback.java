package com.imoonx.file.recycle.callback;
...
import com.imoonx.file.recycle.BaseItemDraggableAdapter;
...
public class ItemDragAndSwipeCallback extends ItemTouchHelper.Callback {
    ...
    public void clearView(RecyclerView recyclerView,
        ...
        if (viewHolder.itemView.getTag(R.id.BaseQuickAdapter_dragging_support) != null && (Boolean) viewHolder.itemView
                .getTag(R.id.BaseQuickAdapter_dragging_support)) {
            mAdapter.onItemDragEnd(viewHolder);
            viewHolder.itemView.setTag(R.id.BaseQuickAdapter_dragging_support, false);
        }
        if (viewHolder.itemView.getTag(R.id.BaseQuickAdapter_swiping_support) != null && (Boolean) viewHolder.itemView
                .getTag(R.id.BaseQuickAdapter_swiping_support)) {
    ...
    /**
     * Set the fraction that the user should move the View to be considered as
     * swiped. The fraction is calculated with respect to RecyclerView's bounds.
     * <p>
     * Default value is .5f, which means, to swipe a View, user must move the
     * View at least half of RecyclerView's width or height, depending on the
     * swipe direction.
     *
     * @param swipeThreshold A float value that denotes the fraction of the View size.
     *                       Default value is .8f .
     */
    ...
    /**
     * Set the fraction that the user should move the View to be considered as
     * it is dragged. After a view is moved this amount, ItemTouchHelper starts
     * checking for Views below it for a possible drop.
     *
     ...
     * </p>
     * <p>
     * The value should be ItemTouchHelper.UP, ItemTouchHelper.DOWN,
     * ItemTouchHelper.LEFT, ItemTouchHelper.RIGHT or their combination.
     * </p>
     * You can combine them like ItemTouchHelper.UP | ItemTouchHelper.DOWN, it
     * means that the item could only move up and down when dragged.
     *
     * @param dragMoveFlags the drag movement direction. Default value is
     ...
     * </p>
     * <p>
     * The value should be ItemTouchHelper.START, ItemTouchHelper.END or their
     * combination.
     * </p>
     * You can combine them like ItemTouchHelper.START | ItemTouchHelper.END, it
     * means that the item could swipe to both left or right.
     *
     * @param swipeMoveFlags the swipe movement direction. Default value is
     *                       ItemTouchHelper.END.
     */
