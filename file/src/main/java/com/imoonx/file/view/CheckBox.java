...
public class CheckBox extends View {
    ...
    public CheckBox(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        normalColor = a.getColor(R.styleable.CheckBox_checkbox_color, Res.getColor(R.color.color_48baf3));
        unCheckedColor = a.getColor(R.styleable.CheckBox_unchecked_color, Res.getColor(R.color.transparent_70_black));
        disabledColor = a.getColor(R.styleable.CheckBox_disabled_color, Res.getColor(R.color.color_e5e5e5));
        borderColor = a.getColor(R.styleable.CheckBox_border_color, Color.WHITE);
        size = a.getDimensionPixelSize(R.styleable.CheckBox_checkbox_size, Res.dip2px(22));
    ...
    private void init() {
        if (paint == null) {
            ...
            textPaint.setTextSize(Res.dip2px(16));
    ...
    private void animateToCheckedState(boolean newCheckedState) {
        ...
        checkAnimator = ObjectAnimator.ofFloat(this, "progress",
                newCheckedState ? 1 : 0);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawBackground || progress != 0) {
            ...
            float checkProgress = progress < 0.5f ? 0.0f
                    : (progress - 0.5f) / 0.5f;

            float roundProgressCheckState = isCheckAnimation ? progress
                    : (1.0f - progress);
            if (roundProgressCheckState < progressBounceDiff) {
                drawRad -= Res.dip2px(2) * roundProgressCheckState
                        / progressBounceDiff;
            } else if (roundProgressCheckState < progressBounceDiff * 2) {
                drawRad -= Res.dip2px(2) - Res.dip2px(2)
            ...
            if (drawBackground) {
                ...
                canvas.drawCircle(paddingLeft + rad, paddingTop + rad, drawRad
                        - Res.dip2px(1), paint);
                canvas.drawCircle(paddingLeft + rad, paddingTop + rad, drawRad
                        - Res.dip2px(1), backgroundPaint);
            }
            paint.setColor(isEnabled() ? normalColor : disabledColor);
            bitmapCanvas.drawCircle(rad, rad, showBorderInCheckState ? rad
                    - Res.dip2px(1.2f) : rad, paint);
            bitmapCanvas.drawCircle(rad, rad,
                    showBorderInCheckState ? (rad - Res.dip2px(1.2f))
                            * (1 - roundProgress) : rad * (1 - roundProgress),
                    eraser);
            ...
            if (textLayout != null) {
                checkCanvas.save();
                checkCanvas.translate(rad - (textWidth) / 2, rad - (textHeight)
                        / 2);
                ...
                checkDrawable.setBounds(x, y + checkOffset, x + w, y + h
                        + checkOffset);
                checkDrawable.draw(checkCanvas);
            }
            eraser2.setStrokeWidth(rad);
            checkCanvas.drawCircle(rad, rad, rad * checkProgress + rad / 2,
                    eraser2);
    ...
    public void setText(String text) {
        if (!TextUtils.isEmpty(text)) {
            textLayout = new StaticLayout(text, textPaint, Res.dip2px(100),
