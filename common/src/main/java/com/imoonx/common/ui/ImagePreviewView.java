...
public class ImagePreviewView extends AppCompatImageView {
    ...
    public ImagePreviewView(Context context, AttributeSet attrs,
        ...
        mScaleDetector = new ScaleGestureDetector(getContext(),
                new ScaleListener());
        mFlatDetector = new GestureDetector(getContext(),
                new FlatGestureListener());
    }
    ...
    private float getDiffX() {
        ...
        return translateLeft >= 0 ? translateLeft
                : getWidth() - translateLeft - mScaledWidth > 0 ? -(getWidth()
                - translateLeft - mScaledWidth) : 0;
    }
    ...
    private float getDiffY() {
        ...
        return translateTop >= 0 ? translateTop
                : getHeight() - translateTop - mScaledHeight > 0 ? -(getHeight()
                - translateTop - mScaledHeight)
                : 0;
    }
        ...
        if (action == MotionEvent.ACTION_UP
            ...
            if (isAutoScale) {
                ...
                if (mScaledWidth >= getWidth() && mDiffX != 0) {
                    ...
                    animator.setFloatValues(translateLeft, translateLeft
                            - mDiffX);
                ...
                // height重置到中间位置
                if (mScaledHeight < getHeight() && mScaledWidth >= getWidth()
                        && mDiffY != 0) {
                    ValueAnimator animator = getResetYAnimator();
                    animator.setFloatValues(translateTop,
                            (getHeight() - mScaledHeight) / 2.f);
                    animator.addUpdateListener(getOnTranslateYAnimationUpdate());
                    animator.start();
                }
    ...
    private void resetDefaultState() {
        ...
        mTranslateYAnimator.setFloatValues(translateTop,
                getDefaultTranslateTop(getHeight(), mBoundHeight));
    ...
    private class ScaleListener extends
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (mOldScaledWidth > getWidth() && getDiffX() != 0
                    || (mOldScaledHeight > getHeight() && getDiffY() != 0))
                return false;
            ...
            translateLeft = getWidth() / 2.f
                    - (getWidth() / 2.f - translateLeft) * mScaledWidth
                    / mOldScaledWidth;
            translateTop = getHeight() / 2.f
                    - (getHeight() / 2.f - translateTop) * mScaledHeight
                    / mOldScaledHeight;
    ...
    private class FlatGestureListener extends
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2,
            ...
            if (mScaledHeight > getHeight()) {
                ...
                /*
                 * if (getDiffY() != 0) { final float disY = (float)
				 * (Math.acos(Math.abs(getDiffY()) / getHeight() * 6) *
				 * distanceY); if (disY == disY) translateTop -= disY; // float
				 * 低值溢出变Nan数值 } else { translateTop -= distanceY * 1.5; }
				 */
            }
            ...
            if (mScaledWidth > getWidth()) {
                ...
                translateLeft = t;
				/*
				 * if (getDiffX() != 0) { final float disX = (float)
				 * (Math.acos(Math.abs(getDiffX()) / getWidth() * 4) *
				 * distanceX); if (disX == disX) translateLeft -= disX; } else {
				 * translateLeft -= distanceX * 1.5; }
				 */
            } else {
                isReachBorder = true;
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
            ...
            if (mBoundHeight * scale > getHeight()) {
                ...
                mResetYAnimator
                        .addUpdateListener(getOnTranslateYAnimationUpdate());
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (scale == 1.f) {
                ...
                ValueAnimator mResetXAnimator = getResetXAnimator();
                ValueAnimator mResetYAnimator = getResetYAnimator();
                mResetXAnimator.setFloatValues(translateLeft,
                        (getWidth() - mBoundWidth * 2.f) / 2.f);
                mResetYAnimator.setFloatValues(translateTop,
                        getDefaultTranslateTop(getHeight(), mBoundHeight * 2));
                mResetXAnimator
                        .addUpdateListener(getOnTranslateXAnimationUpdate());
                mResetYAnimator
                        .addUpdateListener(getOnTranslateYAnimationUpdate());
