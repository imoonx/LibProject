...
public class CirclePagerIndicator extends View implements PagerIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCenterHorizontal) {
            // xOffset += ((width - paddingLeft - paddingRight) - (count *
            // circleAndSpace)) / 2.0f;
    ...
    private int measureWidth(int measureSpec) {
        ...
        if ((specMode == MeasureSpec.EXACTLY) || (mViewPager == null)) {
            ...
            final int count = mViewPager.getAdapter().getCount();
            width = (int) (getPaddingLeft() + getPaddingRight() + (count * 2 * mRadius) + (mIndicatorRadius - mRadius) * 2 + (count - 1) * mIndicatorSpace);
