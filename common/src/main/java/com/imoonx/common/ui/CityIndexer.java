...
public class CityIndexer extends View {
    ...
    // 最大字体大小
    float mMaxFontSize = 48;

    // 提示字体大小
    float mTipFontSize = 52;
...
//	int  mTipFontColor = 0xff3399ff;
    ...
    public CityIndexer(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        initData(context, attrs);
    }

    public CityIndexer(Context context, AttributeSet attrs) {
        super(context, attrs);
        initData(context, attrs);
    }

    public CityIndexer(Context context) {
        super(context);
        initData(null, null);
    }
    ...
    private void initData(Context context, AttributeSet attrs) {
        ...
        if (context != null && attrs != null) {
            ...
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CityIndexer, 0, 0);

            mWidthOffset = a.getDimension(R.styleable.CityIndexer_widthOffset, mWidthOffset);
            mMinFontSize = a.getDimension(R.styleable.CityIndexer_minFontSize, mMinFontSize);
            mMaxFontSize = a.getDimension(R.styleable.CityIndexer_maxFontSize, mMaxFontSize);
            mTipFontSize = a.getDimension(R.styleable.CityIndexer_tipFontSize, mTipFontSize);
            mMaxBezierHeight = a.getDimension(R.styleable.CityIndexer_maxBezierHeight, mMaxBezierHeight);
            mMaxBezierWidth = a.getDimension(R.styleable.CityIndexer_maxBezierWidth, mMaxBezierWidth);
            mMaxBezierLines = a.getInteger(R.styleable.CityIndexer_maxBezierLines, mMaxBezierLines);
            mAdditionalTipOffset = a.getDimension(R.styleable.CityIndexer_additionalTipOffset, mAdditionalTipOffset);
            mFontColor = a.getColor(R.styleable.CityIndexer_fontColor, mFontColor);
            mTipFontColor = a.getColor(R.styleable.CityIndexer_tipFontColor, mTipFontColor);
            a.recycle();
        }
