...
//		}
		for (int i = 0; i < mOverlayOptionList.size(); i++) {
			Overlay overlay = mBaiduMap.addOverlay(mOverlayOptionList.get(i));
			if (i == 0 && getStartExtraInfo() != null) {
				// 起点
				overlay.setExtraInfo(getStartExtraInfo());
			} else if (i == 1 && getEndExtraInfo() != null) {
				overlay.setExtraInfo(getEndExtraInfo());
			}
			mOverlayList.add(overlay);
		}
	}

	public abstract Bundle getStartExtraInfo();

	public abstract Bundle getEndExtraInfo();
	/**
	 * 将所有Overlay 从 地图上消除
	 */
