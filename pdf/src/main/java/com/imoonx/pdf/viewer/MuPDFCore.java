...
public class MuPDFCore {
    ...
    public synchronized void drawPage(Bitmap bm, int pageNum, int pageW, int pageH, int patchX, int patchY,
                                      int patchW, int patchH, Cookie cookie) {
        gotoPage(pageNum);
    ...
    private void flattenOutlineNodes(ArrayList<Item> result, Outline list[], String indent) {
        for (Outline node : list) {
            if (node.title != null)
                result.add(new Item(indent + node.title, node.page));
            if (node.down != null)
