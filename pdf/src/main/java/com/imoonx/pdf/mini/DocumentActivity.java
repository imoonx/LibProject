...
/**
 * 预览pdf getIntent().getStringExtra("pdf_path");
 */
public class DocumentActivity extends Activity implements PDFCallBack {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.document_activity_mini);
        ...
        layoutPopupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            public boolean onMenuItemClick(MenuItem item) {
                ...
                if (itemId == R.id.action_layout_6pt)
                    layoutEm = 6;
                if (itemId == R.id.action_layout_7pt)
                    layoutEm = 7;
                if (itemId == R.id.action_layout_8pt)
                    layoutEm = 8;
                if (itemId == R.id.action_layout_9pt)
                    layoutEm = 9;
                if (itemId == R.id.action_layout_10pt)
                    layoutEm = 10;
                if (itemId == R.id.action_layout_11pt)
                    layoutEm = 11;
                if (itemId == R.id.action_layout_12pt)
                    layoutEm = 12;
                if (itemId == R.id.action_layout_13pt)
                    layoutEm = 13;
                if (itemId == R.id.action_layout_14pt)
                    layoutEm = 14;
                if (itemId == R.id.action_layout_15pt)
                    layoutEm = 15;
                if (itemId == R.id.action_layout_16pt)
                    layoutEm = 16;
    ...
    protected void openDocument() {
        worker.add(new Worker.Task() {
            ...
            public void work() {
                XLog.i(DocumentActivity.class, "open document");
    ...
    protected void checkPassword(final String password) {
        worker.add(new Worker.Task() {
            ...
            public void work() {
                XLog.i(DocumentActivity.class, "check password");
    ...
    protected void loadDocument() {
        worker.add(new Worker.Task() {
            public void work() {
                try {
                    XLog.i(DocumentActivity.class, "load document");
                    String metaTitle = doc.getMetaData(Document.META_INFO_TITLE);
                    if (metaTitle != null)
                        title = metaTitle;
                    isReflowable = doc.isReflowable();
                    if (isReflowable) {
                        XLog.i(DocumentActivity.class, "layout document");
                        doc.layout(layoutW, layoutH, layoutEm);
                    }
                    pageCount = doc.countPages();
                } catch (Throwable x) {
                    XLog.i(DocumentActivity.class, x.getMessage());
                    doc = null;
                    pageCount = 1;
                    currentPage = 0;
                }
        ...
        worker.add(new Worker.Task() {
            public void work() {
                try {
                    ...
                    XLog.i(DocumentActivity.class, "relayout document");
                    doc.layout(layoutW, layoutH, layoutEm);
                    pageCount = doc.countPages();
        ...
        worker.add(new Worker.Task() {
            private void flattenOutline(Outline[] outline, String indent) {
                for (Outline node : outline) {
                    if (node.title != null)
                        flatOutline.add(new Item(indent + node.title, node.page));
                    if (node.down != null)
            ...
            public void work() {
                XLog.i(DocumentActivity.class, "load outline");
    ...
    protected void loadPage() {
        ...
        worker.add(new Worker.Task() {
            ...
            public void work() {
                try {
                    XLog.i(DocumentActivity.class, "load page " + pageNumber);
                    Page page = doc.loadPage(pageNumber);
                    XLog.i(DocumentActivity.class, "draw page " + pageNumber);
