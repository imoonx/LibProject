...
public class PageView extends View implements GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener {
    ...
    protected PDFCallBack pdfCallBack;
    ...
    public void onSizeChanged(int w, int h, int ow, int oh) {
        canvasW = w;
        canvasH = h;
        pdfCallBack.onPageViewSizeChanged(w, h);
    }
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        if (showLinks && links != null) {
            ...
            for (Link link : links) {
                ...
                if (mx >= b.x0 && mx <= b.x1 && my >= b.y0 && my <= b.y1) {
                    if (link.uri != null)
                        pdfCallBack.gotoURI(link.uri);
                    else if (link.page >= 0)
                        pdfCallBack.gotoPage(link.page);
                    foundLink = true;
                    break;
                }
            }
        ...
        if (!foundLink) {
            ...
            if (x <= a) goBackward();
            if (x >= b) goForward();
            if (x > a && x < b) pdfCallBack.toggleUI();
        }
        invalidate();
        return true;
    }
