...
public class CropDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        int width = (int) Res.getScreenWidth();
        int height = (int) Res.getScreenHeight();
        ...
        canvas.drawCircle(rect.left, rect.top + mCropHeight, RADIUS,
                mCirclePaint);
        canvas.drawCircle(rect.right, rect.top + mCropHeight, RADIUS,
                mCirclePaint);
        canvas.drawCircle(rect.left, rect.bottom - mCropHeight, RADIUS,
                mCirclePaint);
        canvas.drawCircle(rect.right, rect.bottom - mCropHeight, RADIUS,
                mCirclePaint);
    }
    ...
    public void setRegion(Rect rect) {
        int width = (int) Res.getScreenWidth();
        int height = (int) Res.getScreenHeight();
        rect.set((width - mCropWidth) / 2, (height - mCropHeight) / 2,
                (width + mCropWidth) / 2, (height + mCropHeight) / 2);
    }
